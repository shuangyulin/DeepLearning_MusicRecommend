...
def messages_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'messages' != 'forum' :
            if req_dict.get("userid") and 'messages' != 'chat' and 'messages' != 'examrecord':
                del req_dict["userid"]
        else:
            if tablename!="users" and tablename!="jdfnl" and 'messages'[:7]!='discuss' and "userid" in messages.getallcolumn(messages,messages):
                req_dict["userid"] = request.session.get("params").get("id")
