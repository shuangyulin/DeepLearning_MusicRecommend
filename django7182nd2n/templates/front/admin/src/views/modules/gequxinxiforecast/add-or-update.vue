	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="作者" prop="zuozhe" >
					<el-input v-model="ruleForm.zuozhe" placeholder="作者" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="收藏" prop="shoucang" >
					<el-input v-model.number="ruleForm.shoucang" placeholder="收藏" clearable  :readonly="ro.shoucang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="收藏" prop="shoucang" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="播放" prop="bofang" >
					<el-input v-model.number="ruleForm.bofang" placeholder="播放" clearable  :readonly="ro.bofang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="播放" prop="bofang" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `gequxinxiforecast/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.gequxinxiforecastCrossAddOrUpdateFlag = false;
											this.parent.search();
