<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('gequxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{gequxinxiCount}}</div>
					<div class="name">歌曲信息总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('fensixinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{fensixinxiCount}}</div>
					<div class="name">粉丝信息总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('gedaninfo','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita6"></span>
				</div>
				<div class="right">
					<div class="num">{{gedaninfoCount}}</div>
					<div class="name">歌单信息总数</div>
				</div>
			</div>
			<div id="statis4" class="statis4 animate__animated" v-if="isAuth('songinfo','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-shijian02"></span>
				</div>
				<div class="right">
					<div class="num">{{songinfoCount}}</div>
					<div class="name">歌曲信息总数</div>
				</div>
			</div>
		</div>
		...
		<!-- 5 -->
		<div class="type5">
			<div id="gequxinxiChart1" class="echarts1 animate__animated" v-if="isAuth('gequxinxi','首页统计')"></div>
			<div id="gequxinxiChart2" class="echarts2 animate__animated" v-if="isAuth('gequxinxi','首页统计')"></div>
			<div id="gequxinxiChart3" class="echarts3 animate__animated" v-if="isAuth('gequxinxi','首页统计')"></div>
			<div id="gequxinxiChart4" class="echarts4 animate__animated" v-if="isAuth('gequxinxi','首页统计')"></div>
			<div id="fensixinxiChart1" class="echarts5 animate__animated" v-if="isAuth('fensixinxi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		if(this.isAuth('fensixinxi','首页统计')){
			this.fensixinxiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'statis4',css:'animate__flipInY'},
				{id:'news-box',css:'animate__bounceInDown'},
				{id:'gequxinxiChart1',css:'animate__fadeInUp'},
				{id:'gequxinxiChart2',css:'animate__fadeInUp'},
				{id:'gequxinxiChart3',css:'animate__fadeInUp'},
				{id:'gequxinxiChart4',css:'animate__fadeInUp'},
				{id:'fensixinxiChart1',css:'animate__fadeInUp'},
			]
...
// 1234 竖
		fensixinxiChat1() {
			this.$nextTick(()=>{
				...
				var fensixinxiChart1 = echarts.init(document.getElementById("fensixinxiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						fensixinxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							fensixinxiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
	...
	.home-content {
		...
		.statis-box {
			...
			.statis1 {
				...
				background: url(http://codegen.caihongy.cn/20241123/d5ab4becbdd444c08cf97412dcc6b2ba.png) no-repeat center top / 100% 100%;
				display: flex;
				width: calc(20% - 20px);
				min-height: 80px;
